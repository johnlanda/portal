...
func RotateCertificates(cfg RotateConfig) (*TunnelMetadata, error) {
	...
	resolvedDir, err := filepath.EvalSymlinks(cfg.TunnelDir)
	if err != nil {
		return nil, fmt.Errorf("failed to resolve tunnel directory path: %w", err)
	}
	cfg.TunnelDir = resolvedDir
	...
	if meta.DeployTarget == "bare-metal" {
		...
		for _, side := range []struct {
			...
			if err := os.WriteFile(filepath.Join(certDir, "tls.crt"), side.cert, 0600); err != nil {
			...
			if err := os.WriteFile(filepath.Join(certDir, "ca.crt"), side.ca, 0600); err != nil {
		...
		if err := os.WriteFile(sourcePath, sourceSecret.Content, 0600); err != nil {
		...
		if err := os.WriteFile(destPath, destSecret.Content, 0600); err != nil {
